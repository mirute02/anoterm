...
> (F-04 / F-12 / F-14 / F-15 / S-02 / S-03 / U-01)、第 3 弾で **F-13 (scrollback メモリバジェット)** を実装済み。
> **安定性・電池に効く F/S 系の主要項目は完了**。自動再接続 + tmux 再アタッチも入っている。
> debug ビルド・ユニットテスト (12 件, 0 失敗) は通過。**残るは大きめの機能追加 U-02〜U-10 と
> 運用整備 S-04〜S-07**（SFTP・ポートフォワード等は安定性ではなく機能拡張）。実機での動作確認が最終工程。
...
## 13. 実装ログ (2026-07-07 第 3 弾) — F-13 メモリ対策

debug ビルド + `testDebugUnitTest`(12 件, 0 失敗)通過。

### 方針判断
設計書の F-13 には「(1) 全体バジェット / (2) Cell のプリミティブ化 / (3) onTrimMemory」があった。
このうち **(2) プリミティブ化は描画・選択・折返し・wide 文字処理すべてに触る大改造で回帰リスクが高く**、
単一の実機検証パスでは検出しきれない恐れがあるため今回は見送り。より安全で LMK に直接効く
**(1) + (3)** を実装した。(2) は将来 GC 圧をさらに下げたくなったときの課題として残す。

### 実装済み

| ID | 変更ファイル | 内容 |
|---|---|---|
| F-13 | `terminal/emulator/TerminalBuffer.kt` | `maxScrollback` を var 化し `setMaxScrollback(limit)` を追加（[MIN=200, ABSOLUTE_MAX=5000] にクランプ、超過行を即トリム）。 |
| F-13 | `terminal/emulator/TerminalEmulator.kt` | `setScrollbackLimit(limit)` を追加（@Synchronized で emulator monitor を取り、primary バッファをトリム。描画の lockless 読取と直列化されるので安全）。 |
| F-13 | `ssh/SshSessionManager.kt` | アプリ全体の scrollback バジェット管理。`ActivityManager.memoryClass` の約 1/6（8〜48MB にクランプ）を予算とし、5 秒ごとにタブ数で等分 → 各タブの横幅から上限行数を算出して動的に締める。少数タブでは実質 2000 行のまま（体験不変）、多タブ×横長で自動的に縮む。`onTrimMemory` で RUNNING_CRITICAL 以上なら scrollback を最小まで落として LMK 前に自衛。 |
| F-13 | `AnotermApp.kt` | `onTrimMemory` を override して SessionManager へ委譲。 |
| S-07(部分) | `app/src/test/.../TerminalEmulatorTest.kt` | `setScrollbackLimit` のトリム挙動と MIN クランプを検証するテストを 2 件追加。 |

### メモリ試算（実装後）
- 単一タブ 80 桁: 上限 2000 行のまま ≈ 5〜6MB（従来どおり）。
- 10 タブ × 200 桁 / 予算 42MB: 各タブ ≈ 525 行に自動縮小、合計 ≈ 42MB に有界化（従来は最大 ~160MB）。

### 残（安定性以外の機能拡張・運用整備）
- S-04 release スモーク / CI、S-05 Keystore 障害耐性、S-06 ホスト鍵変更 UI、S-07 セッション層テストの拡充。
- U-02 通知状態強化、U-03 前回セッション復帰導線、U-04 ポートフォワード、U-05 SFTP、U-06 scrollback 検索、
  U-07 URL タップ、U-08 セッションログ、U-09 バックグラウンド設定ページ、U-10 セルフ診断画面。
- F-13 の (2) Cell プリミティブ化（GC 圧のさらなる低減が必要になったとき）。
