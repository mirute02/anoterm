...
- **ウィンドウ一覧（≡）** — 開いている SSH 接続と、その先の tmux をツリーで出す。
  接続 → セッション → ウィンドウの 3 階層で、**どの接続のどのセッションにいる
  ウィンドウでも 1 タップで飛べる**。別の接続なら、タブの移動・アタッチ先の切り替え・
  ウィンドウの選択をまとめて行う。開いた瞬間だけ全接続を並列に読む
...
- **画像を開く** — 端末に出たパスに下線が付き、押すとその画像が出る。Claude Code や
  ...
  スマホからは中身を確かめようがなかった。読み込みは作業中のシェルとは別の SSH
  セッションで行うので、コマンドラインに `base64` が混ざらない。相対パスは tmux の
  ペインの現在地から探す。ピンチで拡大
- **ページを隣で見る** — 端末に出た `http://` の URL に下線が付き、押すと端末の隣に
  ブラウザが開く。**通信はこの SSH 接続の中を通す**ので、`localhost` にしか口を
  開けていない開発サーバーも、外に晒さずに見られる。待ち受けは端末内の 127.0.0.1 に
  限定してあり、同じ Wi-Fi の誰かが覗ける形にはならない。上下・左右どちらの分割にも
  でき、仕切りをドラッグして配分を変えられる。端末のブラウザに渡して全画面で見ることも
  できる
- **浮かぶ返答パッド** — キーボードを閉じている間、端末の上に 1/2/3 を三角に重ねる。
  承認プロンプトに答えるためだけに IME を開いて本文を隠す必要がなくなる。好きな位置へ
  動かせる
- **打っている間だけ出る補助キー** — Esc・矢印・Ctrl などはキーボードと一緒に出入りする。
  読んでいる間まで置いておくには画面が狭い
