  ...
  Scaffold(
    ...
    Column(
      ...
      Section("公開鍵をサーバに登録する") {
        ...
        Text(
            "PC から `ssh-copy-id user@host` で自動登録できるが、Android から直接使うのは"
                + "手順が多いので AnoTerm 画面で公開鍵コピー → SSH 接続 → echo で追記が現実的。",
        )
      }
      ...
      Section("passphrase について") {
        Text(
            "passphrase は秘密鍵ファイル自体を暗号化するパスワード。"
                ...
                + "モバイル端末では設定を推奨。"
                + "AnoTerm では Android の biometric lock と組み合わせることで、"
      ...
      Section("鍵の形式について（詳細）") {
        ...
        Bullet("**DSA**: 非推奨・使わない")
        Bullet("**OpenSSH 形式 / PEM 形式**: AnoTerm は両方読める。生成時は OpenSSH 形式がデフォルト。")
      }
